...
void BST<T>::insert(const T & t)
{
   ...
   while(nodeTraverse)
   {
      ...
      {
         BNode * insertion = new BNode(t);
         insertion->pParent = nodeTraverse;
         insertion->pLeft = nodeTraverse->pLeft;
         nodeTraverse->pLeft = insertion;

         if(insertion->pLeft)
         {
            insertion->pLeft->pParent = insertion;
         }
...
void BST<T>::BNode::balance()
{
   ...
   if ( 
        ...
        /*( (!sibling->isRed) && (!aunt->isRed) )    &&*/ // it was on the pseudocode
